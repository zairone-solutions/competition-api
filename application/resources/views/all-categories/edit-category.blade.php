                        ...
                        <form method="POST" action="{{ route('updatecategory', $category->id) }}" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="form-group focused col-12 col-lg-6">
									<label for="title" class="form-control-label">{{ __('Title') }}<span class="required-fields">*</span></label>
										<input id="title" type="text" class="form-control form-control-alternative @error('title') is-invalid @enderror" name="title" value="{{ $category->title }}" required>
								...
								<div class="form-group focused col-12 col-lg-6">
									<label for="slug" class="form-control-label">{{ __('Slug') }}<span class="required-fields">*</span></label>
										<input id="slug" type="slug" class="form-control form-control-alternative @error('slug') is-invalid @enderror" name="slug" value="{{ $category->slug }}" required>
							...
							<div class="row">
								<div class="form-group col-12 col-lg-6">
									<label for="verified" class="form-control-label">{{ __('Verified') }}<span class="required-fields">*</span></label>
										<input id="verified" type="checkbox" class="form-control @error('verified') is-invalid @enderror" name="verified" value="1" {{ $category->verified ? 'checked' : '' }}>
								...
								<div class="form-group focused col-12 col-lg-6">
									...
									<select class="form-control form-control-alternative js-example-basic-single" id="suggest_id" name="suggest_id">
										...
										@foreach ($users as $user)
											<option value="{{ $user->id }}" {{ $category->suggest_id == $user->id ? 'selected' : '' }}>{{ $user->full_name }}</option>
										@endforeach
									</select>
									@error('suggest_id')
										<span class="invalid-feedback" role="alert">
