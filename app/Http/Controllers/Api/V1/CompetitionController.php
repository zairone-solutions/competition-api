...
class CompetitionController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $slug_matches = Competition::where("slug", Str::slug($request->title))->count();
    ...
    public function publish(Request $request, Competition $competition)
    {
        ...
        // Email
        @Mail::to(auth()->user())->send(new CompetitionPublished($competition));
    ...
    public function participate(Request $request, Competition $competition)
    {
        ...
        // Email & Notifications
        @Mail::to(auth()->user())->send(new CompetitionParticipation($competition));
        @Mail::to($competition->organizer)->send(new CompetitionParticipationAlert($competition));
        $this->triggerNotification($competition->organizer->id, $competition->organizer->notification_token, "New participation!", 'participation', auth()->user()->username . " has participated in your competition.", ['id' => $competition->id, 'slug' => $competition->slug]);
